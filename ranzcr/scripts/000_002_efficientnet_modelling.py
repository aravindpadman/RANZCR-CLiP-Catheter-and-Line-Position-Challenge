#!/usr/bin/env python
# coding: utf-8
"""investigate the effect of CosineAnnealing learning rate scheduler"""
...
class Trainer:
    ...
    def configure_schedulers(self, **kwargs):
        if self.optimizer:
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, eta_min=0, T_max=10)
        else:
            ...
            if self.scheduler:
                if self.step_scheduler_after == "batch":
                    if self.step_scheduler_metric is None:
                        neptune.log_metric('learning_rate', self.scheduler.get_last_lr()[0])
                        self.scheduler.step()
                    else:
                        pass
...
if __name__ == '__main__':
    ...
    trainer.fit(fp16=True, validation_batch_size=64)
